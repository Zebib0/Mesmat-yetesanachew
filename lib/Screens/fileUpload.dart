  ...
  Future<void> _pickAudioFile() async {
    try {
      ...
      if (result != null) {
        ...
        if (path.endsWith('.mp3') || path.endsWith('.aac') ||
            path.endsWith('.wav')) {
          setState(() {
            _filePath = path;
          });
        } else {
          // Display error message for non-audio file
...
/*
  Future<String> transcribeAudio(String filePath) async {
    var request = http.MultipartRequest(
      'POST',
      Uri.parse('http://127.0.0.1/recognize'), // Adjust the URL to your server's address
    );
